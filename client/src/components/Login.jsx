  ...
  return (
    <div className="h-full w-full flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden -z-10 grid-overlay">
        <div className="absolute -top-40 -left-40 w-[28rem] h-[28rem] rounded-full bg-white/10 blur-3xl orb-drift" />
        <div className="absolute -bottom-40 -right-40 w-[28rem] h-[28rem] rounded-full bg-white/8 blur-3xl orb-drift-slow" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 rounded-full bg-white/5 blur-3xl" />
      </div>
          ...
          {steps.map((s, i) => (
            <span
              ...
              className={`h-1 rounded-full transition-all duration-300 ${
                i === stepIndex ? "w-6 bg-white" : i < stepIndex ? "w-4 bg-white/50" : "w-4 bg-white/15"
              }`}
            />
          ))}
        </div>
          ...
          <form onSubmit={handlePhoneContinue} className="animate-floatIn">
            ...
            <div className="premium-input rounded-xl flex items-center overflow-hidden">
              <select
                ...
                className="bg-transparent text-white/80 pl-4 pr-2 py-3 outline-none border-r border-white/10 text-sm"
              >
                {COUNTRY_CODES.map((c) => (
              ...
              <input
                ...
                className="flex-1 bg-transparent px-3 py-3 text-white placeholder-white/25 outline-none"
                maxLength={16}
              />
            </div>
        ...
        {step === "otp" && (
          <form onSubmit={handleOtpContinue} className="animate-floatIn">
            ...
            <div className="flex gap-3 justify-center">
              {otp.map((digit, i) => (
                <input
                  ...
                  inputMode="numeric"
                  maxLength={1}
                  className="premium-input rounded-xl w-14 h-14 text-center text-xl text-white outline-none"
                />
              ))}
            </div>
            <button type="button" onClick={() => setOtp(["", "", "", ""])} className="text-xs text-white/30 hover:text-white/60 mt-3 block mx-auto transition-colors">
              Clear
            </button>
            ...
            <button
              ...
              className="btn-mirror-ghost mt-3 w-full rounded-xl text-white/60 hover:text-white/90 font-medium py-2.5 text-sm"
            >
              ← Change number
            </button>
          </form>
          ...
          <form onSubmit={handleSubmit} className="animate-floatIn">
            ...
            <div className="premium-input rounded-xl">
              <input
                ...
                className="w-full bg-transparent px-4 py-3 text-white placeholder-white/25 outline-none"
                maxLength={24}
              />
            </div>
